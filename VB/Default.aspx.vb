...
Partial Public Class _Default
    ...
    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs)
        ASPxGridView1.DataSource = Enumerable.Range(0,100).Select(Function(i) New With { _
            Key .ID=i, _
            Key .Name="Product" & i, _
            Key .Description="Some description" & i, _
            Key .Date = If(i Mod 2 = 0, Date.Now.AddDays(i), Date.Now.AddDays(-i)) _
        })
    End Sub
    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs)
        ...
        Select Case command
            ...
            Case Else
        End Select
        Dim criterias = CriteriaColumnAffinityResolver.SplitByColumnNames(CriteriaOperator.Parse(GridView.FilterExpression))
        If Not criterias.Item2.Keys.Contains(field) Then
            criterias.Item2.Add(field, filterExpression)
        Else
            criterias.Item2(field) = filterExpression
        End If
        Dim expression = CriteriaOperator.ToString(GroupOperator.And(criterias.Item2.Values))
